...
class Choice extends React.Component {
    ...
    render() {
        const { classes, children, selected, disabled, idPrefix, testId, leftIcon, rightIcon, id } = this.props;
        ...
        return (
            ...
            >
                {leftIcon && <Icon iconName={leftIcon} colors={[iconColor]} width={iconSizes.small} height={iconSizes.small} />}
                {children && <Text tag="span" className={classes.text} primary={selected} disabled={disabled}>{children}</Text>}
                {rightIcon && <Icon iconName={rightIcon} colors={[iconColor]} width={iconSizes.small} height={iconSizes.small} />}
            </button>
        );
    }
}
