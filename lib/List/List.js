...
const List = ({
  list, style, id, className,
    ...
    const renderText = (text, props, link) => {
      ...
      if (result.length === 1 && linkText.length === 1) return [{text}];
      const child = [];
      if (result.length > 1) {
        for (let i = 0, length = result.length; i < length; i += 1) {
          if (i === 1) child.push({text: result[i], props}); else child.push({text: result[i]});
        }
      }
      if (linkText.length > 1) {
        if (!(result.length > 1)) {
          for (let i = 0, length = linkText.length; i < length; i += 1) {
            if (i === 1) child.push({text: linkText[i], link}); else child.push({text: linkText[i]});
          }
        } else {
          const newChild = [];
          for (let i = 0, length = child.length; i < length; i += 1) {
            ...
            if (patTest) {
              ...
              for (let i = 0, length = resultLink.length; i < length; i += 1) {
                if (i === 1) resultLink[i] = {text: resultLink[i], link}; else resultLink[i] = {text: resultLink[i]};
