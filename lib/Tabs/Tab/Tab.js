...
function Tab(props) {
  ...
  const { title, size, counter, icon, testId, value, disabled, classes } = props;
  ...
  return (
    <button
      ...
      className={`${classes.button} ${classes[size]}${
        value === currentValue ? ` ${classes.selected}` : ''
    ...
    >
      {icon ? (
        <Icon
          size={iconSize}
          iconName={icon}
          className={`${classes.icon}${disabled ? ` ${classes.iconDisabled}` : ''}`}
