                ...
                <ul className={classes.footerList}>
                    {
                        buttons.map(button => (
                            <li key={button.id} className={classes.listElement}>
                                <a className={classes.link} {...button} onClick={'sub' in button ? () => {this.showSub(button);} : undefined}>{button.label}</a>
                                {button.sub && (
                                    <ul className={classes.dropdownMenu} {...button.sub}>
                                        {button.sub.items.map(item => (
                                            <li key={item.id} className={classes.dropdownElement}>
                                                <a className={classes.dropdownLink} {...item}>{item.label}</a>
                                            </li>
                                        ))}
                                    </ul>
                                )}
