...
class Text extends Component {
    ...
    getColor() {
        const { classes, primary, secondary, success, error, warning, info, disabled, white } = this.props;
        const colors = [{primary}, {secondary}, {success}, {error}, {warning}, {info}, {disabled}, {white}];
    ...
    getWeight() {
        ...
        const sizes = [{hero}, {headline}, {heading}, {subheading}, {large}, {standard}, {small}, {micro}];
        const filtered = this.filter(sizes);
        if ((filtered.length == 0 || large || standard || small) && strong) {
    ...
    render() {
        ...
        return (
            <TextType className={`${classes.text} ${size} ${color ? color : emphasis}${topSpacing}${bottomSpacing}${align ? ` ${align}` : ''}${strong ? ` ${strong}` : ''}${className ? ` ${className}` : ''}`} style={style}>{children}</TextType>
        );
    }
}
