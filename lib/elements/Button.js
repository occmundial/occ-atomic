...
class Button extends React.Component {
    render() {
        const { classes, extraClasses, children, id, href, target, onClick, theme, size, block, disabled, icon, customStyles, loading } = this.props;
        if (href) {
            return (
                <a
                    className={`${classes.btn} ${theme ? classes[theme] : ''} ${loading ? classes.loading : ''} ${disabled ? classes.disabled : ''} ${size == "md" || size == "lg" ? classes[size] : ''} ${block ? classes.block : ''} ${extraClasses ? extraClasses : ''}`}
                    href={href}
