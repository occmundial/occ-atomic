...
const Column = ({ classes, children, xs, sm, md, lg, extraClasses, styles }) => (
    <div
        className={`${classes.column}${xs ? getClasses('xs', xs, classes) : ''}${sm ? getClasses('sm', sm, classes) : ''}${md ? getClasses('md', md, classes) : ''}${lg ? getClasses('lg', lg, classes) : ''} ${extraClasses ? extraClasses : ''}`}
        style={styles}>
        {children}
    </div>
);
