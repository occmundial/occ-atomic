...
class SlideToggle extends React.Component {
    ...
    render() {
        ...
        const { classes, label, disabled, id, className, style, name, idCheck } = this.props;
        return (
            <div id={id} className={`${classes.cont}${disabled ? ` ${classes.disabled}` : ''}${className ? ` ${className}` : ''}`} style={style}>
                <div id={idCheck} name={name} className={`${classes.switch}${value ? ` ${classes.checked}` : ''}`} onClick={this.toggle}>
