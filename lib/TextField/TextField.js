...
class TextField extends React.Component {
    ...
    render() {
        const { status, value, touched, showPass } = this.state;
        ...
        if (disabled) {
            return (
                <div className={this.setClassName(realStatus)} style={style}>
                    ...
                    {iconName && (
                        <Icon iconName={iconName} width={24} height={24} display="inline-block" className={classes.icon} colors={[colors.grey100]} />
                    )}
                    {type == 'password' && (
                        <Icon iconName="eye" width={18} height={18} display="inline-block" className={classes.passIcon} colors={[colors.grey100]} />
                    )}
                    <label className={`${classes.input}${iconName ? ` ${classes.hasIcon}` : ''}`}>
                        {(input && input.value) ? input.value : value ? value : placeholder}
                    </label>
                    <div className={classes.bottom}>
                        {assistiveText && <label className={`${classes.label} ${classes.left} ${realStatus == 'error' ? classes.errorAssistiveText : classes.assistiveText}`}>{assistiveText}</label>}
                    </div>
                </div>
            );
        }
        return (
            <div className={this.setClassName(realStatus)} style={style}>
                ...
                {iconName && (
                    <Icon iconName={iconName} width={24} height={24} display="inline-block" className={classes.icon} colors={[colors.grey200]} />
                )}
                {type == 'password' && (
                    <Icon iconName="eye" width={18} height={18} display="inline-block" className={classes.passIcon} colors={showPass ? [colors.grey900] : [colors.grey200]} onClick={this.togglePass} />
                )}
                ...
                <div className={classes.bottom}>
                    {assistiveText && (
                        <label className={`${classes.label} ${classes.left} ${realStatus == 'error' ? classes.errorAssistiveText : classes.assistiveText}`}>{assistiveText}</label>
                    )}
                </div>
            </div>
        );
