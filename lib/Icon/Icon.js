...
class Icon extends React.Component {
    ...
    render() {
        const { classes, className, style, id, name, onClick, alt } = this.props;
        const IconType = onClick ? 'button' : 'span';
        return (
            <IconType id={id} className={`${classes.icon}${onClick ? ` ${classes.click}` : ''}${className ? ` ${className}` : ''}`} name={name} style={style} onClick={onClick} alt={alt} />
        );
    }
}
