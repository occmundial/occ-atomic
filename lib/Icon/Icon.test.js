...
describe("Icon styles", () => {
    ...
    it('returns the icon with a given color', () => {
        let props = { iconName: "testing" };
        expect(styles.icon.background(props)).toBe('url(data:image/svg+xml;base64,dGVzdGluZyAtICMwMDA=)');
        expect(styles.icon['&:hover'].background(props)).toBe(undefined);
        props.colors=['#f00']; props.hover=['#00f'];
        expect(styles.icon.background(props)).toBe('url(data:image/svg+xml;base64,dGVzdGluZyAtICNmMDA=)');
        expect(styles.icon['&:hover'].background(props)).toBe('url(data:image/svg+xml;base64,dGVzdGluZyAtICMwMGY=)');
    });
