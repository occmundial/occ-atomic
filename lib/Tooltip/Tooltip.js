...
function Tooltip({
	...
	const getMiddlewares = useMemo(() => {
		...
		showArrow && middlewares.push(arrow({ element: arrowRef, padding: `${spacing['size-2']} ${spacing['size-3']}` }));
			...
			apply({ elements, rects, availableWidth }) {
				...
				if (fit) {
					...
					if (width){
						...
						styles.minWidth = !(/\b(end|start|left|right)\b/i).test(placement) ? `${(rects.reference.width / 2) + 32}px` : ''
					}
					else styles.width = '';
				}
