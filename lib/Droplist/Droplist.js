...
const Droplist = ({ items, filter, term, isOnFocus, groups, groupNameKey, groupIdKey, groupItemsKey, itemIdKey, itemTextKey, itemTextRightKey, onClick, onEnter, onMouseDown, onMouseUp, className, id, style }) => {
    ...
    const moveToPreviousGroup = () => {
        if (currentGroup === -1) {
            ...
            setCurrentGroup(newGroup);
            if (_items[newGroup][groupItemsKey] && _items[newGroup][groupItemsKey].length)
                setCurrentItem(_items[newGroup][groupItemsKey].length-1);
            ...
            const newGroup = currentGroup-1;
            setCurrentGroup(newGroup);
            if (items[newGroup][groupItemsKey] && items[newGroup][groupItemsKey].length)
                setCurrentItem(items[newGroup][groupItemsKey].length-1);
            else moveToPreviousGroup();
        }
    };

    const moveDown = items => {
    ...
    const renderList = (items, selectedGroup) => {
