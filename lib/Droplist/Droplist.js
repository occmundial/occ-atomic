			...
			if (index >= 0) {
				...
				return (
					<div
						...
						}${!item.disabled ? ` ${classes.itemPointer}` : ''}`}
					>
						<div>
							{item.iconName && (
								<Icon
							...
							<Text className={item.iconName ? classes.iconText : ''} low={item.disabled}>
								{text[0].length ? text[0] : ''}
								<Text tag="b" strong low={item.disabled}>
									{text[1].length ? text[1] : ''}
								</Text>
								{text[2].length ? text[2] : ''}
								{item.extraText && (
									<Text tag="span" strong low={item.disabled} className={`${classes.extraText}${!item.disabled ? ` ${classes.extraTextColor}` : ''}`}>
										{item.extraText}
									</Text>
								)}
							</Text>
