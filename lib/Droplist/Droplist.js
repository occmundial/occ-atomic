...
import { compareText, separateText, groupHasItems } from './functions';
...
class Droplist extends React.Component {
    ...
    onEnter() {
        ...
        const { onEnter, groups } = this.props;
        if (currentItem > -1) {
            if (groups) {
                const selectedGroup = items.find((group, i) => i === currentGroup);
                const selectedItem = selectedGroup.items.find((item, i) => i === currentItem);
    ...
    moveGroupDown() {
        ...
        if (currentGroup === -1) {
            ...
            const groupItems = items[currentGroup].items;
    ...
    moveToNextGroup() {
        ...
        if (currentGroup === items.length-1) {
            ...
            this.setState({ currentGroup: currentGroup+1 }, () => {
                ...
                if (items[currentGroup].items && items[currentGroup].items.length) {
    ...
    moveToPreviousGroup() {
        ...
        if (currentGroup === -1) {
            this.setState({ currentGroup: items.length-1 }, () => {
                const { currentGroup } = this.state;
                if (items[currentGroup].items && items[currentGroup].items.length) {
                    this.setState({ currentItem: items[currentGroup].items.length-1 });
                } else {
                    this.moveToPreviousGroup();
                }
            });
    ...
    filterItems(items) {
        const { itemTextKey, term } = this.props;
    ...
    render() {
        ...
        const { classes, className, style, id, groups, groupNameKey, groupIdKey, groupItemsKey, itemTextKey, term } = this.props;
        return (
            <div className={`${classes.block}${className ? ` ${className}` : ''}`} id={id} style={style}>
                {
                    groups ? (
                        items.map((group, index) => {
                            if (groupHasItems(group, term, groupItemsKey, itemTextKey)) {
                                return (
